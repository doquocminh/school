<?php
    ...
    $archive_layout = $show_no_of_category = $tz_post_class_list = $tz_post_classes = '';
    /* Check archive type */
    ...
    $tz_post_class_list[] = ( $tz_general_archive_column_type == 'four-columns' ) ? 'masonry-four-columns-button': '';
    ...
    ob_start();
        /* To add default post class */
        add_action( 'post_class', 'paperio_archive_masonry_post_class' );
        post_class( $tz_post_class_list );
        remove_action( 'post_class', 'paperio_archive_masonry_post_class' );
            ...
            echo '<div class="blog-meta text-uppercase">';
                if( $tz_archive_hide_read_more_button != 1 ) {
                    if( $tz_archive_default_button_text ) {
                        echo '<a href="'.get_permalink().'" class="btn btn-very-small text-uppercase alt-font archive-button no-letter-spacing'.esc_html( $tz_blog_archive_class ).'">'.esc_attr( $tz_archive_default_button_text ).' '.$tz_archive_read_more_button_arrow.'</a>';
                    }
                }
