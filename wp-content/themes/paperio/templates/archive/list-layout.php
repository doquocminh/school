<?php
    ...
    echo '<div id="post-'.get_the_ID().'" '.$tz_post_classes.'>';
        echo '<div class="col-md-12 col-sm-12 col-xs-12 no-padding blog-listing-style8">';
        if ( !post_password_required() ) {
            if ( has_post_thumbnail() ) {
                if( has_post_thumbnail() ) {
                    echo '<div class="col-md-5 col-sm-6 col-xs-12 margin-six-bottom xs-margin-ten-bottom">';
                        get_template_part( 'loop/archive/content', 'image' );
                    echo '</div>';
                }
            ...
            if ( has_post_thumbnail() ) {
                ...
                echo '<h2 class="alt-font font-weight-600 title-small text-mid-gray entry-title archive-layout-title no-margin-bottom">';
                ...
                if( ( $tz_archive_hide_post_category != 1 ) || ( $tz_archive_hide_post_date != 1 ) ) {
                    ...
                    if ( has_post_thumbnail() ) {
                        ...
                        echo '<div class="letter-spacing-1 text-extra-small text-uppercase margin-two-bottom sm-margin-three-bottom xs-margin-six-bottom display-inline-block archive-layout-meta">';
                    }

                        echo '<ul class="post-meta-box meta-box-style2">';
                            if( $tz_archive_hide_post_category != 1 ) {
                                if( get_post_type( get_the_ID() ) == 'post' ) {
                                    echo '<li>'.paperio_post_category( get_the_ID(), 'text-link-light-gray archive-layout-meta-link', '2' ).'</li>';
                                }
                            }
                ...
                echo '<div class="blog-meta text-uppercase">';
                    if( $tz_archive_hide_read_more_button != 1 ) {
                        echo '<a href="'.get_permalink().'" class="btn btn-very-small text-uppercase alt-font archive-button no-letter-spacing'.esc_attr( $tz_blog_archive_class ).'">'.esc_attr( $tz_archive_default_button_text ).' '.$tz_archive_read_more_button_arrow.'';
                        echo '</a>';
                    }
