...
if( !class_exists('Paperio_Breadcrumb_Navigation') ){
	class Paperio_Breadcrumb_Navigation {
		...
		function __construct() {
			$this->opt = array(
				...
				// Text displayed for the search page.
					'title_search' => esc_html__( 'Search', 'paperio' ),
				...
				// The prefix that is used for mouseover link (e.g.: "Browse to: Archive")
					'urltitle_prefix' => esc_html__( 'Browse to: ', 'paperio' ),
				...
				// Prefix for tags (Simple Tagging Plugin)
					'tag_page_prefix' => esc_html__( 'Tag: ', 'paperio' ),
				// Prefix for tags (Simple Tagging Plugin)
					'tag_page_suffix' => '', 
				// Text displayed for a 404 error page, , only being used if 'use404' => true
					'title_404' => esc_html__( '404', 'paperio' ),
				// For post type
					'title_portfolio' => esc_html__( 'Portfolio', 'paperio' ),
	...
	/*==============================================================================
	    				=== DISPLAY BREADCRUMB ===
	  ============================================================================*/
		function paperio_display_breadcrumb() {
			...
			************************************************* */
			...
			case 'page':
				...
				while( 0 != $bcn_theparentid ) {
					...
					//echo print_r($mylink);
					$bcn_titlearray[$bcn_loopcount] = '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" class="text-link-light-gray" href="' . get_permalink($bcn_theparentid) . '" title="' . $this->opt['urltitle_prefix'] . $mylink->post_title . $this->opt['urltitle_suffix'] . '"><span itemprop="name">' . $mylink->post_title . '</span><meta itemprop="position" content="'.$page_counter.'" /></a></li>';
				...
				$tz_hide_page_breadcrumb_title = get_theme_mod( 'tz_hide_page_breadcrumb_title', 0 );
					...
					if( strtolower($bcn_pagetitle) != strtolower($this->opt['title_home']) ) {  // Check if we are not on home
						...
						if ( $tz_hide_page_breadcrumb_title != 1 ) {
			...
			case 'singlepost':
				...
				$tz_hide_breadcrumb_title = get_theme_mod('tz_hide_breadcrumb_title',0);
				
				if ( $tz_hide_breadcrumb_title != 1 ) {
	        ...
	        case 'categories':
				...
				while ($parent_id) {
					...
					$cat_breadcrumbs = '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" class="text-link-light-gray" href="' . get_category_link($category->cat_ID) . '" title="' . $this->opt['urltitle_prefix'] . $category->cat_name . $this->opt['urltitle_suffix'] . '"><span itemprop="name">' . $category->cat_name . '</span><meta itemprop="position" content="'.$cat_counter.'" /></a></li>' . $this->opt['separator'] . $cat_breadcrumbs;
	...
	} // END class Paperio_Breadcrumb_Navigation
}