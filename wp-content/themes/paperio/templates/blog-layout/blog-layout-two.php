<?php
    ...
    /* Check if blog post quote icon hide / show */
    $tz_hide_blog_post_quotes_icon = ( get_theme_mod( 'tz_hide_blog_post_quotes_icon', 1 ) == 1 ) ? ' hide-quote-icon' : '' ;
        ...
        while ( have_posts() ) : the_post();
            ...
            if ( is_sticky() ) {
                ...
                echo '<div id="post-'.get_the_ID().'" '.$tz_post_classes.'>';
                    echo '<div class="blog-post margin-nine-bottom xs-margin-ten-bottom">';
                        if ( !post_password_required() ) {
                            if( has_post_thumbnail() ) {
                                echo '<div class="blog-image margin-seven-bottom">';
                                    get_template_part( 'loop/blog-layout/content', 'image' );
                                echo '</div>';
                            }
                        }
                        echo '<div class="blog-details comment-icon'.esc_attr( $tz_hide_blog_post_quotes_icon ).'">';
                            echo '<h2 class="alt-font font-weight-600 text-mid-gray no-margin-top margin-one-bottom xs-margin-three-bottom title-small text-uppercase entry-title blog-layout-title">';
                            ...
                            if( $tz_hide_blog_category != 1 || $tz_hide_blog_author != 1 || $tz_hide_blog_date != 1 ) {
                                echo '<div class="text-extra-small text-uppercase letter-spacing-1 margin-three-bottom display-block blog-layout-meta">';
                            ...
                            if( $tz_hide_blog_post_excerpt != 1 ) {
                                if( $tz_blog_post_excerpt_length ) {
                                    ...
                                    if( $show_excerpt ){
                                        echo '<p class="text-medium entry-summary">'.esc_html( $show_excerpt ).'</p>';
                                    }
                                }
                            }
                            if( ( $tz_hide_blog_read_more != 1 && $tz_general_blog_button_text != '' ) || $tz_hide_blog_comment_link != 1 || $tz_hide_blog_like != 1 ) {
                                ...
                                echo '<div class="blog-meta text-uppercase">';
                                    if( ( $tz_hide_blog_read_more != 1 ) && ( $tz_general_blog_button_text != '' ) ) {
                                        echo '<a href="'.get_permalink().'" class="btn btn-color btn-small text-uppercase alt-font text-small text-white">'.esc_html( $tz_general_blog_button_text ).' '.$tz_general_blog_button_arrow.'</a>';
                                    }
                                    if ( $tz_hide_blog_comment_link != 1 || $tz_hide_blog_like != 1 ) {
                                        echo '<ul class="blog-listing-comment fl-right letter-spacing-1 general-blog-listing-comment">';
